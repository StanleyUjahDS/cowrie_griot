import 'package:flutter/material.dart';

class AppColors {
  // 🌑 DARK THEME BASE
  static const Color darkPrimary = Color(0xFF00211A);
  static const Color darkSecondary = Color(0xFF004D3A);
  static const Color darkSurface = Color(0xFF012D23);

  // ☀️ LIGHT THEME BASE
  static const Color lightPrimary = Color(0xFFE6FAF4);
  static const Color lightSecondary = Color(0xFFBFF3E5);
  static const Color lightSurface = Colors.white;

  // 🔘 BRAND ACCENTS (GLOBAL)
  static const Color gold = Color(0xFFF0BA4F);
  static const Color green = Color(0xFF00C896);
  static const Color earth = Color(0xFF6A682B);
}